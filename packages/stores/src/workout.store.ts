...
interface WorkoutSessionState {
    ...
    updateSet:      (localId: string, setId: string,
                    field: "reps" | "weight", value: string)            => void;
...
export const useWorkoutSessionStore = create<WorkoutSessionState>(
  (set, get) => ({
    ...
    // ── Session lifecycle ───────────────────────────────────────────────────
    startSession: (name) =>
      set({ isActive: true, workoutName: name, startedAt: new Date(), activeExercises: [] }),
      ...
      get().activeExercises.reduce(
        (acc, e) =>
          ...
          e.sets
            ...
            .reduce((sAcc, s) => sAcc + parseFloat(s.weight) * parseInt(s.reps), 0),
        0
      ),
  })
);