...
import { getContributionDateRange, processContributions, } from '@gymtracker/utils';
export function useContributions(range) {
    // Derive date strings from range — stable across renders
    // for same range value
    const { from, to } = useMemo(() => getContributionDateRange(CONTRIBUTION_RANGES[range].days), [range]);
    const { data, isLoading, error } = useQuery({
        queryKey: queryKeys.stats.contributions(range),
        queryFn: () => statsApi.getContributions({ from, to, source: 'workouts' })
            .then(r => r.data),
